...
bool stem_analyser(pcl::PointCloud<pcl::PointXYZRGB>::Ptr plant_cloud, float adjacent_dist){
    if(sea_vis_flag) viewer_alg1->removeAllPointClouds();

    printf("SEA Starts.\n");
    printf("Adjacent Distance:%s", std::to_string(adjacent_dist).c_str());
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr plant_cloud(new pcl::PointCloud<pcl::PointXYZRGB>);//创建指针 
